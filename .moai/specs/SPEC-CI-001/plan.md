...
| A **per-file** coverage gate that `--cov-fail-under` cannot express | `conftest.py:204-245`; rationale at `conftest.py:169-185` |
| A declared lint standard reporting **zero findings** | `ruff.toml`; measured with ruff 0.16.1 |
| A buildable image with sensible layer order | `Dockerfile:31-32` before `Dockerfile:43` |
...
| **T3** | **Verify T2 on a branch.** Assert **0 skipped** and a pass count at or above `MIN_PASSED` — the value **in force on the ref under test**, read from `ci.yml`; this document deliberately carries no copy of it, per N5. Assert that **every floor declared in `PER_FILE_COVERAGE_TARGETS` (`conftest.py:204-212`) is met, whatever that set currently is** — the assertion is on the set, not on an enumeration of it. An enumerated list here would be a second copy of the floors, and it would go stale exactly as it did: this task originally named **three** floors, and there are now **six**. The check is the literal line `Per-file coverage gate passed` in the log (`conftest.py:245`), which the hook emits with the live set enumerated in it — compare the log's enumeration against `conftest.py`, not against this table. **This is the first execution of this codebase on x86_64**; every prior measurement, including all of SPEC-MEMORY-001's, was aarch64 (`.moai/specs/SPEC-MEMORY-001/spec.md:160`). Read the architecture **out of the run log** — `ubuntu-latest` is a label, not a measurement. Treat the result as data, not as a formality. **Record: `.moai/specs/SPEC-CI-001/verification-T3.md`.** | `verification-T3.md` | T2 |
| **T4** | **Write the `image` job.** `docker compose build coderunner` with `type=gha` cache; then `docker run --rm --entrypoint python coderunner-ai:latest -c "import main, memory, recall, vectorstore, tools"`; then SHA-256 compare the five `/app` files named at `Dockerfile:43` against the checkout. No pytest inside the container (N6; see `spec.md` §3.2 for the three accommodations avoided). | `.github/workflows/ci.yml` | T1 |
...
`conftest.py:204-212` holds the real floors. The workflow's entire relationship with coverage
...
the only one: a host job installing both requirements files runs the whole suite including
