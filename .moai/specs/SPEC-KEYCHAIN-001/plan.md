...
|---|---|
| `params.py` and `settings.py` in the image | `Dockerfile:34` copies five modules; measured `ls /app` lists six entries, neither module among them; `import params` in the image raises `ModuleNotFoundError` |
...
| **T7** | **`Dockerfile:34` — add three modules, not one.** The line currently reads `COPY main.py tools.py memory.py recall.py vectorstore.py ./`. Add `keychain.py` **and** `params.py` **and** `settings.py`. Measured 2026-08-07: the latter two are absent from `coderunner-ai:latest` and `import params` inside it raises `ModuleNotFoundError`; the fault is latent only because the built image predates SPEC-INPUT-001 (`/app/main.py` is 1045 lines against the tree's 1227), which is `product.md` §6.3 exactly. **Verify by rebuilding and importing**, not by reading the diff — the whole point of this task is that reading the line was not enough last time. | `Dockerfile` | T1 |
...
|---|---|---|
| **High** | T3, T5, T7, T8 | T3 is where the capture policy is silently disabled or not; T5 is where every existing user's launcher breaks or not; T7 is where this SPEC's module — and two others — reach the image or not; T8 is the only thing that makes any of it checkable |
...
| **R3** | **`Dockerfile:34` is edited for `keychain.py` and the two missing modules are not noticed.** Measured 2026-08-07: `params.py` and `settings.py` are absent from `coderunner-ai:latest`; `import params` inside the image raises `ModuleNotFoundError`. The image only starts because it is stale (`/app/main.py` is 1045 lines, pre-SPEC-INPUT-001) — `product.md` §6.3. | **Live now, independent of this SPEC**, and the next `docker compose build` surfaces it as a container that dies at import with no diagnostic beyond a traceback. This SPEC is the first work to touch that line since the omission landed. | **T7 fixes all three names and verifies by rebuilding and importing**, not by reading the diff. AC-IMAGE asserts that every first-party module `main.py` imports is present in the built image, derived from `main.py`'s import list rather than from a hand-maintained second list — so the next module to be added fails this test instead of shipping missing. |
