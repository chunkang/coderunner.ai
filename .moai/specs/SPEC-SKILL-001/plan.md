...
Six tasks, one prerequisite, and a gate in the middle. **None has been started.**
...
|---|---|---|---|
| **R1** | **The meter's numbers are not the ones being optimised.** `prompt_eval_count` is what the *server* charged for the prompt it received. Ollama caches prompt prefixes across a session, so a repeated prefix may be counted without being re-evaluated — a saving that shows in the meter and not on the clock, or the reverse. | **The most likely way this SPEC produces a confident wrong number.** It looks like a measurement because it is one; it is just not a measurement of the thing the reader will assume. | T2 records `prompt_eval_count` **and** `total_duration` in the same row, and `acceptance.md` AC-METER requires the record to state which one a given claim rests on. A token saving that does not move wall-clock is still a saving — of context-window budget — and it must be labelled as that rather than as speed. |
