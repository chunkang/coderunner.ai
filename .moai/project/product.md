...
| 5 | **Diagnostic report** — `./coderunner --doctor` prints 15 fields (OS/arch, docker binary, docker version, compose command, daemon reachability, image presence, ollama service status, pulled models, ollama volume mountpoint, **app volume mountpoint**, **chat model presence**, **embedding model presence**, bootstrap log path, **keychain backend**, **stored secret names and count**) and exits 0. The keychain fields report names only, never a value. The count is asserted by `test_doctor_prints_fifteen_fields` | `--doctor` branch | `coderunner:646-714` |
